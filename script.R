...
model <- glm(dias_hospitalizacion ~ Resultado.de.Virus.sincitial.respiratorio + Género + rango_meses+hospital+Nació.prematuro....37.semanas., data = add_lab, na.action=na.fail, family = "poisson")
...
simulationOutput4 <- simulateResiduals(fittedModel = model, plot = T)
...
a<-ggplot(prop_df, aes(rango_meses, prop_uci, group=1))+
  ...
  ylab("Percentage of patients admitted to ICU")+
...
b<-ggplot(prop_oxy,
       aes(x = hospital,
           y = prop_oxy)) +
  geom_col(color="black", linewidth=1) +
  ylab("Proportion of patients that\n required oxygen supplementation")+
  xlab("Hospital")+
  theme_minimal(base_size=16)
b
ggsave("rango_edad_oxy.jpg", b, dpi=300, width = 8, height = 4.5)
...
d<-ggplot(prop_def4, aes(rango_meses, prop_def4, group=1))+
  ...
  facet_wrap(~hospital)+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),limits = c(0, 1))+
  ylab("Percentage of patients that meet \nthe composite severity definition")+
  xlab("Age group (months)")+
  theme_bw(base_size = 14)
d
...
prop_df$Género <-
  recode(prop_df$Género,
         "Masculino" = "Male",
         "Femenino" = "Female")



a<-ggplot(prop_df, aes(Género, prop_oxy))+
  geom_col()+
  facet_wrap(~hospital)+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  ylab("Percentage of patients that required \noxygen supplementation")+
  xlab("Sex")+
  theme_bw(base_size = 14)
a
...
a<-ggplot(prop_df, aes(hospital, prop_oxy))+
  geom_col(color="black")+
  scale_y_continuous(labels = scales::percent_format(accuracy = 1))+
  ylab("Percentage of patients that meet the \ncomposite severity definition")+
  xlab("Hospital")+
  theme_bw(base_size = 14)
a
ggsave("def_4_hospital.jpg", a, dpi=300, width = 8, height = 4.5)
